...
void init_triton_to_cudatile_passes(py::module &&m) {
  ...
  m.def("add_polycube_block_order", [](mlir::PassManager &pm,
                                        std::vector<int64_t> unit_points_flat,
                                        int64_t f_val,
                                        int64_t tm_dim,
                                        int64_t tn_dim,
                                        int64_t sk_dim) {
    std::string encoded;
    for (size_t i = 0; i < unit_points_flat.size(); ++i) {
      if (i)
        encoded.push_back(',');
      encoded += std::to_string(unit_points_flat[i]);
    }
    pm.addPass(mlir::triton::createPolycubeBlockOrderPass(
        std::move(encoded), f_val, tm_dim, tn_dim, sk_dim));
  });
}
