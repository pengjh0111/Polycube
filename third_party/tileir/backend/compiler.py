...
class TileIRBackend(BaseBackend):
    ...
    def make_tileir(mod, metadata, opt: TileIROptions, capability):
        ...
        polycube_cfg = metadata.get("polycube_cfg")
        if polycube_cfg:
            tileir.passes.add_polycube_block_order(
                pm,
                polycube_cfg.get("unitPointsFlat", []),
                polycube_cfg.get("fVal", 0),
                polycube_cfg.get("tmDim", 1),
                polycube_cfg.get("tnDim", 1),
                polycube_cfg.get("skDim", 1),
            )
        passes.common.add_inliner(pm)
